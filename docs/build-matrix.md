...
| Build ID | Builder toolkit | Exact PyTorch | Wheel version | Matching ComfyUI image |
| --- | --- | --- | --- | --- |
...
The image is SSH-ready and accepts newline-separated public keys through `SSH_AUTHORIZED_KEYS` (or `PUBLIC_KEY`). Password login is disabled. Its default command is foreground `sshd`; an explicit container command runs normally. Setting `START_SSHD=1` starts SSH alongside that command.

CPU Pod orchestration sets `RUNPOD_SELF_TERMINATE_SECONDS` to the hard timeout plus cleanup grace. The entrypoint accepts only 600–21600 seconds, requires a valid `RUNPOD_POD_ID`, Pod-scoped `RUNPOD_API_KEY`, and the checksum-pinned `runpodctl` bundled in the image, then arms a detached deletion watchdog. Missing prerequisites fail startup, and credentials are never printed.
...
No GPU is needed to compile or inspect a wheel. A build requires a finite cgroup memory limit, at least 4 effective CPUs, a 32 GiB hard memory limit, enough current headroom for the compiler reserve plus one job, and 20 GiB free disk. A 64 GiB Pod is recommended. The default is conservative: two compiler jobs and one extension at a time. The live 2-by-2 proof peaked near 29.65 GiB, so higher parallelism should only be enabled on a larger Pod.
